...
void main_v1(int n_iter, Matrix * matrix_src, Matrix * matrix_dst, int n_threads)
{
    ...
    if (regions == NULL) {
        printf(">> [version1.c] main_v1: inefficient malloc causing abortion...\n");
        return;
    }
...
void process_v1(Matrix* matrix_src, Matrix* matrix_dst, Region ** r_list, int n_threads, int n_iter)
{
    ...
    if (args == NULL) {
        printf(">> [version1.c] process_v1: inefficient malloc causing abortion...\n");
        return;
    }
    ...
    for (int i = 0; i < n_threads; i++) {
        ...
        // Saving Argument_v1
        // (else the next Argument_v1 will have the same address
        // and erase the previous Argument_v1 values)
        args[i] = a;

        // Launching thread
        pthread_create(&t[i], NULL, thread_func_v1, &args[i]);
    }
